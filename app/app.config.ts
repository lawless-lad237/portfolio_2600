export default defineAppConfig({
  ui: {
    colors: {
      primary: 'emerald',
      neutral: 'zinc'
    }
  },
  data: {
    name: '2600 Studios by Charles Dupilpil',
    photo: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-10.webp',
    title: 'Photographer',
    bio: 'I create minimalist, high-contrast visual stories for brands, people, and events.',
    website: 'https://2600studios.com',
    phone: '+61 469 053 763',
    email: 'hello@2600studios.com',
    x: '',
    facebook: 'https://www.facebook.com/2600studiosbycharles',
    instagram: 'https://www.instagram.com/2600.studios/',
    tiktok: 'https://www.tiktok.com/2600studios/',
    github: 'https://github.com/cesswhite/hato-minimal-template',
    gallery: [
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-0.webp',
        alt: 'Gallery item 1'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-1.webp',
        alt: 'Gallery item 2'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-2.webp',
        alt: 'Gallery item 3'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-3.webp',
        alt: 'Gallery item 4'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-5.webp',
        alt: 'Gallery item 5'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-8.webp',
        alt: 'Gallery item 6'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-9.webp',
        alt: 'Gallery item 7'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-11.webp',
        alt: 'Gallery item 8'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-12.webp',
        alt: 'Gallery item 9'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-4.webp',
        alt: 'Gallery item 10'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-6.webp',
        alt: 'Gallery item 11'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-7.webp',
        alt: 'Gallery item 12'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-10.webp',
        alt: 'Gallery item 13'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-11.webp',
        alt: 'Gallery item 14'
      },
      {
        url: 'https://d3ynb031qx3d1.cloudfront.net/general-demos/demo-gallery-2.webp',
        alt: 'Gallery item 15'
      }
    ]
  }
})
